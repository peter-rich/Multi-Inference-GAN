...
def w_generative_network(z, input_dim, n_layer, n_hidden, eps_dim):
    with tf.variable_scope("generative_w"):
        eps = standard_normal([z.get_shape().as_list()[0], eps_dim], name="eps") * 1.0
        h = tf.concat([z, eps], 1) 
        h = slim.repeat(h, n_layer, slim.fully_connected, n_hidden, activation_fn=tf.nn.relu)
        w = slim.fully_connected(h, input_dim, activation_fn=None, scope="p_w")
    return w
...
def w_inference_network(w, latent_dim, n_layer, n_hidden, eps_dim):
    with tf.variable_scope("inference_w"):
        eps = standard_normal([w.get_shape().as_list()[0], eps_dim], name="eps") * 1.0
        h = tf.concat([w, eps], 1) 
        h = slim.repeat(h, n_layer, slim.fully_connected, n_hidden, activation_fn=tf.nn.relu)
        z = slim.fully_connected(h, latent_dim, activation_fn=None, scope="q_wz")
    return z
...
def w_data_network(w,z, n_layers=2, n_hidden=256, activation_fn=None):
    """Approximate w log data density."""
    h = tf.concat([w,z], 1)
    with tf.variable_scope('discriminator_w'):
        h = slim.repeat(h, n_layers, slim.fully_connected, n_hidden, activation_fn=tf.nn.relu)
        log_d = slim.fully_connected(h, 1, activation_fn=activation_fn)
    return tf.squeeze(log_d, squeeze_dims=[1])