...
q_xzy = graph_replace(p_y, {z: q_xz})#p_y
q_yzx = graph_replace(p_x, {z: q_yz})#p_x

decoder_logit_zy = graph_replace(decoder_logit_y, {p_y: q_xzy, z:q_xz})
encoder_logit_zy = encoder_logit_y

decoder_logit_zx = graph_replace(decoder_logit_x, {p_x: q_yzx, z:q_yz})
encoder_logit_zx = encoder_logit_x

decoder_loss_zy = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.zeros_like(decoder_logit_zy), logits=decoder_logit_zy)
encoder_loss_zy = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.ones_like(encoder_logit_zy), logits=encoder_logit_zy)

decoder_loss_zx = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.zeros_like(decoder_logit_zx), logits=decoder_logit_zx)
encoder_loss_zx = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.ones_like(encoder_logit_zx), logits=encoder_logit_zx)

disc_loss_zy = tf.reduce_mean(decoder_loss_zy) + tf.reduce_mean(encoder_loss_zy)
disc_loss_zx = tf.reduce_mean(decoder_loss_zx) + tf.reduce_mean(encoder_loss_zx)

disc_loss_xy = disc_loss_zx + disc_loss_zy

rec_xzyz = graph_replace(rec_yz, {p_y: q_xzy})#rec_yz
rec_yzxz = graph_replace(rec_xz, {p_x: q_yzx})#rec_xz

cost_xzyz = tf.reduce_mean(tf.pow(rec_xzyz - q_yz, 2))# cost_yz
cost_yzxz = tf.reduce_mean(tf.pow(rec_yzxz - q_xz, 2))# cost_xz

decoder_loss2_zy = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.ones_like(decoder_logit_zy), logits=decoder_logit_zy)
encoder_loss2_zy = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.zeros_like(encoder_logit_zy), logits=encoder_logit_zy)

decoder_loss2_zx = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.ones_like(decoder_logit_zx), logits=decoder_logit_zx)
encoder_loss2_zx = tf.nn.sigmoid_cross_entropy_with_logits(labels = tf.zeros_like(encoder_logit_zx), logits=encoder_logit_zx)

gen_loss_xzy = tf.reduce_mean(decoder_loss2_zy) + tf.reduce_mean(encoder_loss2_zy)
gen_loss_yzx = tf.reduce_mean(decoder_loss2_zx) + tf.reduce_mean(encoder_loss2_zx)
gen_loss_xy = 1.*gen_loss_xzy + 1.*gen_loss_yzx + 1.0*cost_xzyz + 1.0*cost_yzxz + 1.0*cost_x + 1.0*cost_y

train_gen_op_xy =  opt.minimize(gen_loss_xy, var_list= qvars + pvars + qvars_y + pvars_y)
train_disc_op_xy = opt.minimize(disc_loss_xy, var_list= dvars + dvars_y)
...
for epoch in tqdm( range(n_epoch_2), total=n_epoch_2):
    ...
    for xmb, ymb, zmb in iter_data(X_dataset, Y_dataset, Z_dataset, size=batch_size):
        i = i + 1
        for _ in range(1):
            f_d_xy, _ = sess.run([disc_loss_xy, train_disc_op_xy], feed_dict={x: xmb, y:ymb, z:zmb})
        for _ in range(5):
            f_g_xy, _ = sess.run([[gen_loss_xy, gen_loss_xz, cost_x, cost_xz], train_gen_op_xy], feed_dict={x: xmb, y:ymb, z:zmb})
        FG_xy.append(f_g_xy)
        FD_xy.append(f_d_xy)
